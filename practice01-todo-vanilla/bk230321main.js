...
const createDoingList = (text) => {
  ...
  doneButton.addEventListener('click', () => {
    ...
    backButton.addEventListener('click', () => {
      ...
      // 未完了リストから親要素 li を削除
      deleteTargetChildElm(doneTaskLists, backTarget);
      // タスクテキストを取得
      const backTargetText = backTarget.firstElementChild.firstChild.innerText;

      // 親要素 li の中身を初期化
      backTarget.textContent = null;
      // div を生成
      const backTaskContentElm = document.createElement('div');
      // 親要素の配下に div を配置
      backTarget.appendChild(backTaskContentElm);
      // p を生成、クラス付与、p の中身のテキストを追加
      const backTaskTextElm = document.createElement('p');
      backTaskTextElm.className = 'todo-title';
      backTaskTextElm.innerText = backTargetText;
      // 戻すボタンを生成
      // const backButton = document.createElement('button');
      // backButton.innerText = '戻す';
      backTaskContentElm.appendChild(backTaskTextElm);
